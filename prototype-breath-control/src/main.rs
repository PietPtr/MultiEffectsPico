...
fn setup_dual_adc_and_dac(sys_freq: HertzU32) -> ! {
    ...
    loop {
        let amplification = U8F8::from_bits(PRESSURE.load(Ordering::Relaxed) as u16);
        effect.settings.amplification = amplification;
        ...
        for (aux_sample, jack_sample) in next_aux_rx_buf.iter_mut().zip(next_jack_rx_buf.iter_mut())
        {
            ...
            *jack_sample <<= 1;
            ...
            // mix / apply effects
            let jack_sample_as_i1f15 = I1F15::from_bits((signed_jack_sample >> 16) as i16);
            let jack_with_effect = effect.next(jack_sample_as_i1f15);
            let signed_jack_sample = (jack_with_effect.to_bits() as i32) << 16;
...
fn main() -> ! {
    ...
    let mut bmp = bmp280_rs::BMP280::new(
        &mut i2c,
        bmp280_rs::I2CAddress::SdoPulledUp,
        bmp280_rs::Config {
            measurement_standby_time_millis: Some(MeasurementStandbyTimeMillis::ZeroPointFive),
            pressure_oversampling: bmp280_rs::PressureOversampling::One,
            temperature_oversampling: bmp280_rs::TemperatureOversampling::One,
            iir_filter: bmp280_rs::IIRFilterCoefficient::Off,
        },
    )
    .unwrap();

    loop {
        _ = bmp.trigger_measurement(&mut i2c);
        let Ok(raw_pressure) = bmp.read_raw_pressure(&mut i2c) else {
            continue;
        };

        let amplification = raw_pressure.saturating_sub(277_000);
        let amplification = amplification;
        let amplification = 65536 - amplification.clamp(1, 65535);

        PRESSURE.store(amplification / 20, Ordering::Relaxed);
        info!("amp {}", amplification);
